...
def build_skin_assessment(
    ...
    candidate_ulcer_probability = 0.0
    for index, candidate in enumerate(candidates):
        ...
        for member in ensemble["members"]:
            if member["model_name"] == candidate["model_name"]:
                ...
                break
        candidate_ulcer_probability += candidate_weight * (float(candidate["abnormal_probability"]) / 100.0)
    ...
    return {
        ...
        "visual_features": {
            ...
            "ulcer_ratio": visual_features.get("ulcer_ratio", visual_features["dark_ratio"]) * 100,
            "inflamed_ratio": visual_features.get("inflamed_ratio", visual_features["redness"]) * 100,
            "center_ulcer_ratio": visual_features.get("center_ulcer_ratio", visual_features["contrast"]) * 100,
            "healthy_ratio": visual_features.get("healthy_ratio", 1.0 - visual_features["dark_ratio"]) * 100,
            "center_uniformity": visual_features.get("center_uniformity", 1.0 - visual_features["contrast"]) * 100,
            "smooth_texture": visual_features.get(
                "smooth_texture",
                1.0 - min(visual_features["edge_density"] / 0.2, 1.0),
            ) * 100,
        },
    }


def generate_debug_scores_plot(debug_scores: dict[str, float], output_path: Path) -> None:
    labels = [
        "Candidate Ulcer",
        "Visual Ulcer",
        "Visual Healthy",
    ...
    values = [
        debug_scores["candidate_ulcer_probability"],
        debug_scores["visual_ulcer_probability"],
        debug_scores["visual_healthy_probability"],
        debug_scores["final_ulcer_score"],
        debug_scores["final_healthy_score"],
    ]
    colors = ["#ae4f2f", "#c56b47", "#2f6f4f", "#7f351d", "#4b8b66"]

    figure, axis = plt.subplots(figsize=(8.5, 4.2), facecolor="#fffaf4")
    axis.set_facecolor("#f7efe6")

    bars = axis.bar(labels, values, color=colors, edgecolor="#d6c2a8", linewidth=1.2)
    axis.set_ylim(0, 100)
    axis.set_ylabel("Score (%)", color="#2b211c")
    axis.set_title("Debug Scores", color="#2b211c", fontsize=13, pad=12)
    axis.grid(axis="y", linestyle="--", linewidth=0.8, alpha=0.35, color="#715f53")
    axis.tick_params(axis="x", rotation=14, colors="#2b211c")
...
def compute_estimated_classification_metrics(
    candidates: list[dict],
    ensemble: dict,
    skin_assessment: dict,
) -> dict[str, float]:
    """
    Estimate Accuracy, Precision, Recall, and F1-score for a single uploaded image.

    True supervised metrics require a labeled validation/test dataset. This app processes
    one uploaded image at a time, so these values are confidence/agreement-based
    estimates intended for UI comparison graphs, not formal validation metrics.
    """
    predicted_is_abnormal = skin_assessment["label"] == "Ulcer Skin"
    predicted_probability = (
        float(skin_assessment["abnormal_probability"])
        if predicted_is_abnormal
        else float(skin_assessment["normal_probability"])
    ) / 100.0

    candidate_votes = []
    for candidate in candidates:
        abnormal_probability = float(candidate["abnormal_probability"]) / 100.0
        candidate_votes.append(abnormal_probability >= 0.5)

    if candidate_votes:
        agreement = sum(vote == predicted_is_abnormal for vote in candidate_votes) / len(candidate_votes)
    else:
        agreement = 0.0

    confidence_component = predicted_probability
...
def generate_classification_metrics_plot(metrics: dict[str, float], output_path: Path) -> None:
    ...
    bars = axis.bar(labels, values, color=colors, edgecolor="#d6c2a8", linewidth=1.2)
    axis.set_ylim(0, 100)
    axis.set_ylabel("Metric (%)", color="#2b211c")
    axis.set_title("Estimated Classification Metrics", color="#2b211c", fontsize=13, pad=12)
    axis.grid(axis="y", linestyle="--", linewidth=0.8, alpha=0.35, color="#715f53")
...
def analyze_uploaded_image(image_path: Path) -> tuple[list[dict], dict, dict, dict, float]:
    ...
    classification_metrics = compute_estimated_classification_metrics(candidates, ensemble, skin_assessment)
    return candidates, ensemble, skin_assessment, classification_metrics, total_elapsed_ms
...
def index():
    ...
    if request.method == "POST":
        ...
        if image is None or image.filename == "":
            ...
            try:
                candidate_results, ensemble_result, skin_assessment, classification_metrics, inference_ms = analyze_uploaded_image(save_path)
                if skin_assessment is not None:
                    debug_plot_path = save_path.with_name(f"{save_path.stem}_debug_scores.png")
                    generate_debug_scores_plot(skin_assessment["debug_scores"], debug_plot_path)
                    debug_plot_url = f"/uploaded_images/{debug_plot_path.name}"
                if classification_metrics is not None:
                    metrics_plot_path = save_path.with_name(f"{save_path.stem}_classification_metrics.png")
                    generate_classification_metrics_plot(classification_metrics, metrics_plot_path)
                    metrics_plot_url = f"/uploaded_images/{metrics_plot_path.name}"
